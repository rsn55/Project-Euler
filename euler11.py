...
"""In the 20x20 grid below, four numbers along a diagonal line have been marked red.
...
for row in range(17):
    for i in range(20):
        product = grid[row][i]*grid[row-1][i]*grid[row-2][i]*grid[row-3][i]
        if product>maxProd:
            maxProd = product
#check right/left
for row in range(20):
    for i in range(17):
        product = grid[row][i]*grid[row][i+1]*grid[row][i+2]*grid[row][i+3]
        if product>maxProd:
            maxProd = product
#check diagonals
for row in range(20):
    for i in range(20):
        #down left diagonal
        if (i not in [0,1,2,]) and (row not in [17,18,19]):
            product = grid[row][i]*grid[row-1][i-1]*grid[row-2][i-2]*grid[row-3][i-3]
            if product>maxProd:
                maxProd = product
        #down right diagonal
        if (i not in [17,18,19]) and (row not in [17,18,19]):
            product = grid[row][i]*grid[row-1][i+1]*grid[row-2][i+2]*grid[row-3][i+3]
            if product>maxProd:
                maxProd = product
print maxProd
